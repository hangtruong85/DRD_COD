...
class DatasetWithDepth(Dataset):
    """
    Dataset for CAMO with RGB + Depth + Mask support
    ...
    def __init__(self, root, split="train", img_size=256, augment=True, 
        ...
        """
        Args:
            root: Path to CAMO root directory
        ...
        # Logging
        log_msg = f"[CAMO Dataset] {split_dir}: images={len(self.img_paths)}, masks={len(self.mask_paths)}"
        if self.use_depth:
    ...
    def __getitem__(self, idx):
        ...
        # CAMO uses 255 for object, 0 for background
...
class DatasetCAMO(Dataset):
    """
    Simplified Dataset for CAMO with RGB + Mask support (no depth)
    ...
    def __getitem__(self, idx):
        ...
        # Binarize mask (CAMO uses 255 for object, 0 for background)
        mask = (mask > 127).astype("float32")
