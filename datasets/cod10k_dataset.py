...
class DatasetWithDepth(Dataset):
    """
    Dataset for COD10K-v3 with RGB + Depth + Mask support
    Uses ReplayCompose to ensure synchronized augmentation
    
    Directory structure:
        COD10K-v3/
        ├── Train/
        │   ├── Image/          # RGB images (.jpg, .png)
        │   ├── GT_Object/      # Object segmentation masks (.png)
        │   ├── depth/          # Depth maps (.png)
        │   ├── GT_Instance/    # Instance segmentation (optional)
        │   └── GT_Edge/        # Edge maps (optional)
        ├── Val/
        │   ├── Image/
        │   ├── GT_Object/
        │   ├── depth/
    ...
    def __init__(self, root, split="train", img_size=256, augment=True, 
        ...
        """
        Args:
            root: Path to COD10K-v3 root directory
        ...
        self.img_dir = os.path.join(root, split_dir, "Image")
        self.mask_dir = os.path.join(root, split_dir, "GT_Object")
        ...
        # Logging
        log_msg = f"[COD10K-v3 Dataset] {split_dir}: images={len(self.img_paths)}, masks={len(self.mask_paths)}"
        if self.use_depth:
    ...
    def __getitem__(self, idx):
        ...
        # Load mask (GT_Object) - keep original 0-255 range for augmentation
        ...
        # COD10K uses 255 for object, 0 for background
...
class DatasetCOD10K(Dataset):
    """
    Simplified Dataset for COD10K-v3 with RGB + Mask support (no depth)
    ...
    def __getitem__(self, idx):
        ...
        # Binarize mask (COD10K uses 255 for object, 0 for background)
        mask = (mask > 127).astype("float32")
